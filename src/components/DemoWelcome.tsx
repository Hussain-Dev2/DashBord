...
const TOUR_SLIDES = [
  ...
  },
  {
    ...
    title: 'Add, Edit & Organize Clients',
...
export function DemoWelcome() {
  ...
  const WELCOME_MSG = "Hey there! 👋 I'm Nexa, your guide. Welcome to DebtTrack — the smart way to manage clients and payments. Want a quick tour of what this app can do?"

  const { displayed: typedWelcome, done: welcomeDone } = useTypedText(WELCOME_MSG, 20, phase === 'welcome' && visible)
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-end md:items-center justify-center p-4 transition-all duration-500 ${
      ...
      <div className="relative w-full max-w-lg z-10 animate-in fade-in slide-in-from-bottom-8 duration-500">
        ...
        <div className="relative bg-[#0d0f1a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center gap-3 px-6 pt-6 pb-4 border-b border-white/5">
            <div className="flex items-center gap-2.5">
              <div className="relative h-10 w-10 rounded-2xl bg-gradient-to-br from-amber-400 to-yellow-600 flex items-center justify-center shadow-lg shadow-amber-500/20">
                ...
                <div className="absolute -top-0.5 -right-0.5 h-3 w-3 rounded-full bg-green-400 border-2 border-[#0d0f1a]" />
              </div>
              <div>
                <p className="text-sm font-black text-white">Nexa</p>
                <p className="text-[10px] text-amber-400/80 font-bold uppercase tracking-wider">Your App Guide</p>
              </div>
            </div>
            <div className="ms-auto flex items-center gap-3">
              {phase === 'tour' && (
                <span className="text-xs text-gray-500 font-bold tabular-nums">
                  {slideIndex + 1} / {TOUR_SLIDES.length}
                </span>
              )}
              <button
                onClick={handleClose}
                className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 text-gray-500 hover:text-white transition-all group"
              >
          ...
          <div className="p-6">
            {phase === 'welcome' && (
              <div className="space-y-6">
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 bg-white/[0.04] border border-white/5 rounded-2xl rounded-tl-sm px-5 py-4">
                ...
                {/* Buttons — shown only after typing is done */}
                {welcomeDone && (
                  <div className="flex flex-col sm:flex-row gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 min-h-[52px] rounded-2xl font-black text-sm uppercase tracking-wider transition-all active:scale-95 hover:shadow-lg hover:shadow-amber-500/15 shadow-inner"
            ...
            {phase === 'tour' && slide && (
              <div key={slideKey} className="space-y-5 animate-in fade-in slide-in-from-right-8 duration-400">
                {/* Slide badge & icon */}
                ...
                <div className="flex items-center justify-center gap-2 pt-1">
                  {TOUR_SLIDES.map((_, i) => (
                    <button
                      key={i}
                      onClick={() => { setSlideDir(i > slideIndex ? 'right' : 'left'); setSlideKey(k => k+1); setSlideIndex(i) }}
                ...
                <div className="flex gap-3">
                  {slideIndex > 0 && (
                    <button
                      onClick={goPrev}
                      className="flex items-center gap-2 px-5 py-3.5 rounded-2xl text-sm font-bold text-gray-400 bg-white/[0.03] border border-white/5 hover:bg-white/[0.07] hover:text-white transition-all active:scale-95"
                    >
            ...
            {phase === 'bye' && (
              <div className="text-center py-6 space-y-3 animate-in fade-in duration-300">
                ...
                <p className="text-white font-black text-lg">Great! Have fun exploring 🎉</p>
                <p className="text-gray-500 text-sm">Tap the <span className="text-amber-400 font-bold">chat bubble</span> at the bottom-right anytime you need help.</p>
              </div>
            )}
          </div>
        </div>
