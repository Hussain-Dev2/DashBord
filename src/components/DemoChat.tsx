...
const FAQ_ITEMS = {
  en: [
    ...
    },
    {
      ...
      answer: `Absolutely. Open any client's detail view and click **"Generate Invoice"**. You'll get a professional PDF-ready invoice including all payment history.`,
    },
  ],
  ar: [
    {
      id: 'add-client',
      question: '➕ كيف أضيف عميلاً؟',
      answer: `اضغط على زر **"إضافة عميل"** الذهبي في أعلى الشاشة (أو زر ✚ في الجوال). أدخل الاسم ورقم الهاتف والتفاصيل المالية. يمكنك أيضاً إضافة شعار وحقول مخصصة من قسم الإعدادات المتقدمة!`,
    },
    {
      id: 'real-data',
      question: '🧪 هل هذه البيانات حقيقية؟',
      answer: `لا! في **وضع التجربة**، جميع العملاء والأرقام هي بيانات نموذجية لتتصفح بحرية دون أي مخاطر. سجّل دخولك بحساب المشرف لإدارة بيانات حقيقية متزامنة مع السحابة.`,
    },
    {
      id: 'payments',
    ...
    },
    {
      ...
      answer: `بالطبع! استخدم **محدد العملة** في الشريط العلوي للتبديل بين الدولار والدينار العراقي. تُحوَّل المبالغ تلقائياً بأسعار الصرف الحية.`,
    },
    {
      id: 'invoice',
      question: '📄 هل يمكنني إنشاء فواتير؟',
      answer: `بالتأكيد. افتح صفحة تفاصيل أي عميل واضغط **"إنشاء فاتورة"**. ستحصل على فاتورة احترافية جاهزة للطباعة تشمل كامل سجل المدفوعات.`,
    },
  ],
}
...
const UI_TEXT = {
  en: {
    ...
    status: 'Online — always here to help',
    greeting: "Hi! 👋 I'm Nexa. Pick a question below and I'll answer it instantly!",
    commonQuestions: 'Common Questions',
    restartTour: 'Restart the App Tour',
    askAnother: 'Ask another question',
  },
  ar: {
    title: 'مساعد نيكسا',
    status: 'متاح دائماً للمساعدة',
    greeting: 'مرحباً! 👋 أنا نيكسا. اختر سؤالاً أدناه وسأجيب عليه فوراً!',
    commonQuestions: 'الأسئلة الشائعة',
    restartTour: 'إعادة جولة التعريف',
...
export function DemoChat() {
  ...
  const faqList = FAQ_ITEMS[language]
  ...
  const handleFaqClick = (faq: typeof faqList[0]) => {
    setShowFaq(false)
  ...
  return (
    <>
      ...
      {/* Chat Panel — fixed above the button */}
      {isOpen && (
        <div
          dir={isRTL ? 'rtl' : 'ltr'}
          className="fixed bottom-40 md:bottom-24 end-4 md:end-6 z-[89] w-[calc(100vw-2rem)] max-w-sm animate-in fade-in slide-in-from-bottom-4 duration-300"
        >
          ...
          <div className="relative bg-[#0d0f1a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5 bg-white/[0.02]">
              <div className="relative h-9 w-9 rounded-xl bg-gradient-to-br from-amber-400 to-yellow-600 flex items-center justify-center shrink-0 shadow-md shadow-amber-500/20">
                ...
                <div className="absolute -top-0.5 -end-0.5 h-2.5 w-2.5 rounded-full bg-green-400 border-2 border-[#0d0f1a]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black text-white truncate">{ui.title}</p>
                <p className="text-[10px] text-green-400 font-bold">{ui.status}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {messages.length > 0 && (
                  <button onClick={handleReset} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 hover:bg-white/10 text-gray-600 hover:text-gray-300 transition-all" title="Start over">
                    <RotateCcw className="h-3.5 w-3.5" />
                  </button>
                )}
                <button onClick={() => setIsOpen(false)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 hover:bg-white/10 text-gray-600 hover:text-white transition-all group">
              ...
              {messages.length === 0 && (
                <div className="flex gap-2.5 items-start">
                  ...
                  <div className="flex-1 bg-white/[0.04] border border-white/5 rounded-2xl rounded-ss-sm px-4 py-3">
                    <p className="text-sm text-gray-300 leading-relaxed">{ui.greeting}</p>
                  </div>
                </div>
              )}
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-2.5 items-end animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.type === 'question' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                    msg.type === 'question'
                      ? 'bg-amber-500/15 border border-amber-500/20 text-amber-100 rounded-ee-sm font-medium'
                      : 'bg-white/[0.04] border border-white/5 text-gray-300 rounded-ss-sm'
                  }`}>
            ...
            <div className="px-4 pb-4 space-y-2">
              {showFaq ? (
                <div className="space-y-1.5">
                  <p className="text-[10px] text-gray-600 font-bold uppercase tracking-widest px-1 mb-2 text-start">{ui.commonQuestions}</p>
                  {faqList.map((faq) => (
                    <button
                      key={faq.id}
